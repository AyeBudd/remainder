...
import { nativeSocialProviders } from "./social";
...
const baseURL = explicitBaseURL ?? {
  allowedHosts: [...previewAllowedHosts, ...vercelAllowedHosts, "localhost", "127.0.0.1", "[::1]"],
...
const trustedOrigins: string[] = [
  ...vercelOrigins,
  ...vercelAllowedHosts,
  ...vercelAllowedHosts.map((host) => `https://${host}`),
  ...previewAllowedHosts,
  ...previewAllowedHosts.flatMap((host) => [`https://${host}`, `http://${host}`]),
  ...LOCAL_DEV_ORIGINS,
].filter((v, i, a) => Boolean(v) && a.indexOf(v) === i);

const socialProviders = nativeSocialProviders();
...
export const auth = betterAuth({
  ...
  account: {
    ...
    accountLinking: {
      enabled: true,
      trustedProviders: [...GROK_PROVIDERS.map((p) => p.providerId), "google", "twitter"],
